...
| POST | `/auth/login` | `api` | body: `email` (required, email), `password` (required, string) | `200` `{user: AuthUserResource, token}` | `401 {message}` on bad credentials. If the account resolves to `role: admin`, the session is logged out and this endpoint also returns `401` — admins must use `/admin/auth/login`. Sets `first_login_at` on first successful login. |
...
|---|---|---|---|---|
| GET | `/dashboard` | — | `200` inline JSON: `{isEmpty, stats:{monthlyIncome, occupancyPct, occupiedCount, unitCount, outstanding, outstandingCount, expiringCount}, incomeSeries:[{key:"YYYY-MM", amount}] (12 entries, oldest first), needsAttention:[{kind, title, meta, link}]}` | One aggregated payload — no separate Resource class. `needsAttention.kind` ∈ `overdue \| expiring \| notice_given \| ticket_new \| ticket_reopened`, in that priority order. Mirrors `frontend/app/composables/useDashboard.ts` mock computation; must be kept in lock-step by hand. |
...
- **`AuthUserResource`** — `id, name, email, phone?, role?, permissions: string[] (admin only — [] for owner/tenant), isSuperAdmin: bool`
...
- **`PropertyResource`** — `id, ownerId, name, internalLabel?, type?, notes?, address, city, state, postcode, yearBuilt?, builtUpSqft?, landSqft?, bedrooms?, bathrooms?, parkingLots?, furnishing?, ownership? (raw array), utilities? (raw array), coOwners: PropertyCoOwnerResource[], createdAt?`
